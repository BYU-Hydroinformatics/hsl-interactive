...
class Datapoint extends MY_Controller {
	...
	private function createDP($date, $time, $val, $siteid, $varid, $methid, $sourceid) {
		...
		// Run SC update only for this specific SC record rather than running it for the whole thing
		$this->updateSC($siteid, $varid, $methid, $sourceid, $this->cNull($this->config->item('QualityControlLevelID'))))
		return $dataPoint;
	}
	...
	public function addvalue() {
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if (!empty($errors)) {addError($errors);}
		} else {
	...
	public function updateSC($SiteID, $VariableID, $MethodID, $SourceID, $QualityControlLevelID) {
		...
		/*  $db = mysql_select_db($this->config->item('database_name'),$connection)
			or die("<p>Error selecting the database " . $this->config->item('database_name') .
			  mysql_error() . "</p>");
		*/
		require_once APPPATH . '../assets/update_sc_single.php';
	}
}
